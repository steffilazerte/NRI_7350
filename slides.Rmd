...
f <- "slides"
after_date <- lubridate::as_date(c("2021-09-15", "2021-09-22", "2021-09-29", 
                                   "2021-10-06", "2021-10-13", "2021-10-20"))
after_date <- max(which(after_date < Sys.Date()))

if(!dir.exists(f)) dir.create("slides")

file.copy(from = c("_labs/pres_styles.css", 
...
if(length(after_date) > 0){
  ...
  after <- list.files(path = "_labs/Handouts", pattern = ".pdf", full.names = TRUE) %>%
    str_subset("answers") %>%
    str_subset(glue_collapse(glue("_labs/Handouts/{0:after_date} "), sep = "|"))
  file.copy(after, f, overwrite = TRUE)
  
  # HTML
  if(length(after) > 0) {
    after %>% 
      str_remove_all("_labs/Handouts/|.pdf") %>%
      glue_collapse(sep = "|") %>%
      list.files(path = "_labs",  full.names = TRUE, pattern = .) %>%
...
**rOpenSci and Beyond** ([html](https://steffilazerte.ca/Presentations/2021-11%20UofM%20-%20rOpenSci/2021-11_UofM_rOpenSci.html))
